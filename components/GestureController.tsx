...
export const GestureController: React.FC<GestureControllerProps> = ({ onModeChange, currentMode, onHandPosition }) => {
  ...
  useEffect(() => {
    ...
    const drawHandSkeleton = (landmarks: any[], isPinching: boolean, isAiming: boolean) => {
      ...
      // 捏合绿色，瞄准中(未捏合)青色，普通金色
      let color = '#D4AF37';
      if (isPinching) color = '#00FF00';
      else if (isAiming) color = '#00FFFF'; // Cyan for Aiming
    ...
    const detectGesture = (landmarks: any[]) => {
      ...
      // ⚡️ 核心改动：分离 Pinch(捏合) 和 Aiming(瞄准) 阈值
      const PINCH_THRESHOLD = 0.08;
      const AIMING_THRESHOLD = 0.25; // 允许松开到 0.25 依然算瞄准

      const isThumbIndexClose = pinchDist < PINCH_THRESHOLD; 
      const isThumbIndexAiming = pinchDist < AIMING_THRESHOLD; // 宽松的判定

      // 判定逻辑
      const isPinchingNow = isThumbIndexClose && otherFingersExtendedCount >= 2;
      const isAimingNow = isThumbIndexAiming && otherFingersExtendedCount >= 2; // OK手型，但没捏死
      ...
      // 优先级 2: 冷却期
      if (Date.now() - lastPinchReleaseTime.current < 1000) {
        setGestureStatus("..."); 
        return;
      }

      // === 优先级 3: 模式切换 ===
      if (totalExtendedCount >= 4 && !isThumbIndexAiming) { // 拇指食指离远点才算 Open
        openFrames.current++;
        closedFrames.current = 0;
